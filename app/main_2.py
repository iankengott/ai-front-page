    ...
    for i in labels:
        if len(i) > 0:
            ...
            if len(list(i[0]['xmin'])) > 0:
                xmin = int(list(i[0]['xmin']))
                ymin = int(list(i[0]['ymin']))
                xmax = int(list(i[0]['xmax']))
                ymax = int(list(i[0]['ymax']))
                ...
                cv2.putText(ret_image, str(label[0]), (xmin, ymin-10), cv2.FONT_HERSHEY_SIMPLEX, .3, (36,255,12), 1)
